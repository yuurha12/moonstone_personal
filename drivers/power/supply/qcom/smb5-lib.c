...
#define PASSTHROUGH_MODE	BIT(0)

static int smblib_set_passthrough(struct smb_charger *chg, bool enable)
{
	int rc = 0;
	u8 val;

	rc = smblib_read(chg, USBIN_ADAPTER_ALLOW_OVERRIDE_REG, &val);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read USBIN_ADAPTER_ALLOW_OVERRIDE_REG rc=%d\n", rc);
		return rc;
	}

	if (enable)
		val |= PASSTHROUGH_MODE;
	else
		val &= ~PASSTHROUGH_MODE;

	rc = smblib_write(chg, USBIN_ADAPTER_ALLOW_OVERRIDE_REG, val);
	if (rc < 0) {
...
static int smblib_is_input_present(struct smb_charger *chg, int *present)
{
    ...
    *present |= pval.intval ? INPUT_PRESENT_DC : INPUT_NOT_PRESENT;
    ...
    // Check if any input source is present
    if (*present & (INPUT_PRESENT_USB | INPUT_PRESENT_DC)) {
        // Enable passthrough charging
        rc = smblib_set_passthrough(chg, true);
        if (rc < 0) {
            pr_err("Couldn't enable passthrough charging rc=%d\n", rc);
            return rc;
        }
    } else {
        // Disable passthrough charging
        rc = smblib_set_passthrough(chg, false);
        if (rc < 0) {
            pr_err("Couldn't disable passthrough charging rc=%d\n", rc);
            return rc;
        }
    }
