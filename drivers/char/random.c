...
static DEFINE_PER_CPU(struct crng, crngs) = { .generation = ULONG_MAX };
...
static DEFINE_PER_CPU(struct batched_entropy,
		      batched_entropy_u64) = { .position = UINT_MAX };
...
static DEFINE_PER_CPU(struct batched_entropy,
		      batched_entropy_u32) = { .position = UINT_MAX };
...
int __init random_init(const char *command_line)
{
	...
	for (i = 0, arch_bytes = BLAKE2S_BLOCK_SIZE; i < BLAKE2S_BLOCK_SIZE;
	     i += sizeof(entropy)) {
...
static ssize_t urandom_read(struct file *file, char __user *ubuf, size_t len,
			    loff_t *ppos)
{
...
static ssize_t random_read(struct file *file, char __user *ubuf, size_t len,
			   loff_t *ppos)
{
	int ret;
...
static int proc_do_rointvec(struct ctl_table *table, int write,
